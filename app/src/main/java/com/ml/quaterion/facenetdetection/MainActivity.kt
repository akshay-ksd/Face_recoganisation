...
//            alertDialog.show()
    ...
    private fun getStoredUserName(): String {
        ...
        return preferences.getString(PREF_USER_NAME, "Default Name") ?: "Default Name"
    }
    private  fun  launchImage(){
        val preferences = PreferenceManager.getDefaultSharedPreferences(this)
        val path = "imageFilePath"
        val filePath = preferences.getString(path, null)
        ...
        if (fileUri != null) {
            ...
            try {
                val name = getStoredUserName()
                images.add( Pair( name , getFixedBitmap( fileUri ) ) )
            }catch (e: Exception) {
                // Handle any exceptions that may occur during image processing.
                Logger.log("Error while processing images: ${e.message}")
                errorFound = true
            ...
            val alertDialog = AlertDialog.Builder( this ).apply {
                ...
                setMessage( "No users found.Go to user add screen and register" )
                setCancelable( false )
